...
export default function Home() {
  ...
  const updatePick = (team: string, slotIndex: number, patch: Partial<Pick>) => setBoard((current) => {
    ...
    return { ...current, [team]: (current[team] || SLOTS.map(emptyPick)).map((pick, index) => { if (index !== slotIndex) return pick; const next = { ...pick, ...patch }; if (patch.jugador !== undefined) next.posicion = playerMap.get(normalizedPlayerName(patch.jugador))?.posicion || (patch.jugador.trim() ? next.posicion : ""); if (next.jugador.trim() && next.precio !== "") setLastPick({ team, slot: SLOTS[slotIndex], jugador: next.jugador, precio: next.precio, posicion: next.posicion, updatedAt: Date.now() }); return next; }) };
  });
  ...
  const importJSON = async (event: ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; try { const data = JSON.parse(await file.text()); if (!data.board) throw new Error(); const duplicate = findDuplicatePlayer(data.board); if (duplicate) { setToast(`No se importó: ${duplicate} aparece más de una vez.`); event.target.value = ""; return; } setBoard(data.board); if (Array.isArray(data.teams)) setTeams(data.teams); setLastPick(data.lastPick || null); setToast("Board importado."); } catch { setToast("El archivo no es válido."); } event.target.value = ""; };
